...
```
上游 gfwlist ──────────┐
第三方清单（可选）─────┼─► build.mjs ─┬─► public/proxy.pac ─► Cloudflare Pages / jsDelivr
rules/custom-*.txt ────┘              │
...
> 当前数据：4276 条代理域名 / 66 条直连，
> PAC 体积 79.3 KB，最后构建 2026-08-22。
...
**Cloudflare Pages Git 集成（推荐）**：构建命令留空，输出目录 `public/proxy.pac` 的上级（`public`）。
Actions 每天把结果提交回仓库，Pages 自动重新部署，零 Secret。

**wrangler 直推**：配 Secret `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID`
和 Variable `CF_PAGES_PROJECT`。workflow 里 `--branch=main` 不能省，
...
上面这些地址是由 `docs/README.template.md` 里的 `ivanphz/gfwlist-pac` / `main` 占位符
生成的，仓库改名后下一次构建会自动跟着变 —— **但这只解决文档里的地址是新的**，
已经填进 Windows 设置里的那个 URL 不会自动更新。

### ⚠️ 缓存 12 小时

jsDelivr 对分支型 URL（`@main`）最长缓存 12 小时。workflow 里已经加了
...
1. **给 URL 加版本号** —— Windows 的 PAC 缓存按 URL 做键，URL 一变必然重拉：
   ```
   https://cdn.jsdelivr.net/gh/ivanphz/gfwlist-pac@main/public/proxy.pac?v=20260805
   ```
...
当前是 **`direct`**，在 config 里用 `"ipv6"` 切换。
...
拿 `direct-list.txt`（111,442 条，就是 geosite:cn）和当前 gfwlist 实测比对：
