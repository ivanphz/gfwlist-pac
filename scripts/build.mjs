  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    if (/^\[?[0-9a-f]*:[0-9a-f:.]*\]?(\/\d{1,3})?$/i.test(line) && line.includes(':')) {
      if (line.includes('/')) {
        ...
        if (full) b.v6.add(full.split(':').join(''));
        else bad.push(raw.trim());
      }
      continue;
    }
...
function buildPac(cfg, chain, defaultAction, d, tplRaw) {
  const smart = (cfg.ipv6 || 'direct').toLowerCase() === 'smart';

  // 挑选 //#region 段：选中的保留（去掉标记行），没选中的整段删掉
  const keep = smart ? 'V6_SMART' : 'V6_SIMPLE';
  const drop = smart ? 'V6_SIMPLE' : 'V6_SMART';
  let tpl = tplRaw.replace(
    new RegExp(`^//#region ${drop}\\n[\\s\\S]*?^//#endregion ${drop}\\n`, 'gm'),
    ''
  );
  tpl = tpl.replace(new RegExp(`^//#(?:end)?region ${keep}\\n`, 'gm'), '');

  const subs = {
    __PROXY_CHAIN__: chain.replace(/"/g, '\\"'),
    __DEFAULT_ACTION__: defaultAction,
    __PROXY_F__: jsObject(d.proxy.exact),
    __DIRECT_F__: jsObject(d.direct.exact),
    __PROXY_D__: jsObject(d.proxy.domain),
    __DIRECT_D__: jsObject(d.direct.domain),
    __PROXY_N__: jsNets(d.proxy.cidr),
    __DIRECT_N__: jsNets(d.direct.cidr),
    __PROXY_G__: jsArray(d.proxy.glob),
    __DIRECT_G__: jsArray(d.direct.glob),
    __PROXY_K__: jsArray(d.proxy.keyword),
    __DIRECT_K__: jsArray(d.direct.keyword),
    __PROXY_V6__: jsObject(d.proxy.v6),
    __DIRECT_V6__: jsObject(d.direct.v6),
    __PROXY_V6N__: jsArray(d.proxy.v6net),
    __DIRECT_V6N__: jsArray(d.direct.v6net),
  };
  for (const [k, v] of Object.entries(subs)) tpl = tpl.split(k).join(v);

  const left = tpl.match(/__[A-Z0-9_]+__/g);
  if (left) throw new Error(`PAC 模板还有未替换的占位符: ${[...new Set(left)].join(', ')}`);
  ...
  const header = [
    ...
    `ipv6     : ${smart ? 'smart（按规则分流）' : 'direct（字面量一律直连）'}`,
...
async function main() {
  ...
  if ((cfg.ipv6 || 'direct').toLowerCase() !== 'smart'
    ...
    console.warn(
      `[warn] rules 里有 ${proxy.v6.size + direct.v6.size + proxy.v6net.size + direct.v6net.size} 条 IPv6 规则，` +
        '但 ipv6 模式是 direct（字面量一律直连），这些规则不会写进 PAC。' +
        '要启用请在 config 里设 "ipv6": "smart"。'
    );
  }
  const pac = buildPac(cfg, chain, defaultAction, { proxy, direct }, tplRaw);
  ...
  const stats = {
    ...
    ipv6: (cfg.ipv6 || 'direct').toLowerCase(),
    counts: {
